...
class AIChatTheme(BaseTheme):
    ...
    def render_static(self, data):
        ...
        img.paste(frame, (16, 8), frame)  # Icon up
        ...
        if show_chat:
            if state == "smiling" and data.get("bot_response"):
                draw_pixel_text(draw, 8, 48, data["bot_response"][:15], color=(255, 255, 255), spacing=1)
                if self.bubble_frame:
                    img.paste(self.bubble_frame, (7, 47), self.bubble_frame)
            elif state == "error":
                draw_pixel_text(draw, 8, 48, "ERR!", color=(255, 255, 255), spacing=1)
        else:
            if state == "smiling":
    ...
    def animate_frame(self, data, frame_index, static_bg):
        ...
        if show_chat:
            if state == "thinking" and data.get("message"):
                ...
                x_offset = 64 - ((frame_index * 3) % (len(message) * 4 + 64))
                draw_pixel_text(draw, x_offset, 40, message, color=(255, 255, 255), spacing=1)
