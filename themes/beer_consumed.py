...
class BeerConsumedTheme(BaseTheme):
    ...
    def render_static(self, data):
        ...
        if beer_frames:
            consumed = int(data.get('beers_week', 0))  # Ensure integer conversion
        ...
        # Draw stats at separate positions
        self._draw_stats(draw, data, text_color)
        return img

    def _draw_stats(self, draw, data, text_color):
        stats = [
            f'W:{self.format_kpi(data.get("beers_week", 0))}',
            f'M:{self.format_kpi(data.get("beers_month", 0))}',
            f'T:{self.format_kpi(data.get("beers_total", 0))}'
        ]
        
        # Draw stats at distinct y-coordinates to avoid overlap
        draw.text((2, 20), stats[0], fill=text_color, font=self.font)  # Week
        draw.text((2, 30), stats[1], fill=text_color, font=self.font)  # Month
        draw.text((2, 40), stats[2], fill=text_color, font=self.font)  # Total

    def animate_frame(self, data, frame_index, static_bg):
        # For now, keep it static; return the static background
        return static_bg